...
public:
    // Constructor
    Plane(glm::vec3 position, glm::vec3 scale, glm::vec3 rotation, glm::vec3 color)
        : position(position), scale(scale), rotation(rotation), color(color)
    {
        init_position = position;
        ambient = glm::vec3(0.25f, 0.f, 0.f);
    ...
    Plane(glm::vec3 position, glm::vec3 scale, glm::vec3 rotation, glm::vec3 color, glm::vec3 ambient, glm::vec3 diffuse, glm::vec3 specular, float shininess)
        : position(position), scale(scale), rotation(rotation), color(color), ambient(ambient), diffuse(diffuse), specular(specular), shininess(shininess)
    {
        create_vertices();
    }
