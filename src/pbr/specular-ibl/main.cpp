...
int main()
{
    ...
    Shader irradianceShader("cubemap.vs", "light_cubemap.fs");
    ...
    // GENERATING MIP MAPPP
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    glGenerateMipmap(GL_TEXTURE_CUBE_MAP);

    // GENERATING IRRADIANCE MAPPP
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    ...
    {
        // rezise fb according to mip size
...
unsigned int loadHDR(char const *path)
{
    ...
    int width, height, nrComponents;
    float *data = stbi_loadf(path, &width, &height,
                             &nrComponents, 0);
